...
class ChatRoomEncryptionStatusButton extends StatelessWidget with WatchItMixin {
  ...
  IconData getRoomIcon(bool encrypted, Room room) =>
      switch ((encrypted, room.spaceParents.isNotEmpty, room.canonicalAlias)) {
        (true, _, _) => YaruIcons.shield_filled,
        (false, true, String alias) when alias.contains(':ubuntu.com') =>
          YaruIcons.ubuntu_logo_simple,
        (false, _, String alias) when alias.contains(':gnome.org') =>
          YaruIcons.gnome_logo,
        (false, false, _) => YaruIcons.shield_warning,
        (false, true, _) => YaruIcons.globe,
      };
}
